...
const firstCharactersOf = function (strings) {
  return strings.map(function (string) { return string.charAt(0); });
};

// truth values of [0, 1, 2, 3] => [false, true, true, true]
// Assume non-zero numbers are true, and zero is false
const truthValuesOf = function (numbers) {
  return numbers.map(function (number) { return Boolean(number); });
};
...
const domainNamesOf = function (emails) {
  return emails.map(function (email) { return email.split("@")[1]; });
};
...
const getVowelCount = function (init, char) {
  return isCharVowel(char) ? init + 1 : init;
};
